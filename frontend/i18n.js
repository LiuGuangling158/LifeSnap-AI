(function () {
  ...
  const en = {
    ...
    "会话有效": "Session Active",
    "建立会话": "Create Session",
    "管理员会话未建立；保存时会用密钥自动建立短期会话。": "Admin session is not active. Saving will create a short-lived session from the key.",
    "管理员会话已建立，后续管理操作会优先使用短期 token。": "Admin session created. Later admin actions will prefer the short-lived token.",
